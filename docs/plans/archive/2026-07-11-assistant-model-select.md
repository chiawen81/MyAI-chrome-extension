...
- 狀態：**已完成（2026-07-11，手動驗收通過）**；驗收時發現 spike「不污染帳號預設」結論不完整，修正見 §2.1 註記與 §2.4
...
> **驗收後修正（2026-07-11）**：「URL 參數持久性 ✅ 不污染帳號預設」結論**不完整**——spike 只驗證了「開 `?model=` 分頁」不改預設，但 claude.ai 會以「上次**使用**（送出）的模型」作為新對話預設；交棒送出後，後續手動開的新對話仍可能預設為交棒所選模型。spike 當日未觀察到是因為送出測試（urlsend）恰好用了帳號原本的預設模型（Haiku 4.5）。此為 claude.ai 自身的記憶行為、非本外掛可控，已列 FEATURES.md 已知限制；機制選擇不受影響（URL 參數仍優於 DOM 操作——後者是「切換當下」就即時改寫伺服器端預設）。
...
### 2.4 驗收標準（2026-07-11 驗收紀錄）

- [x] options「Claude 助手」分頁每張動作卡都有「交棒模型」下拉，預設「跟隨 claude.ai 目前選擇」；改選後重開設定頁值保留
- [x] 為某動作選「Haiku 4.5」→ 右鍵該動作 → 新分頁模型按鈕顯示 Haiku 4.5，自動送出後以該模型回覆
- [x] **依動作各選生效**：兩個動作設不同模型（如翻譯＝Haiku、摘要此頁＝Sonnet）→ 各自開出的分頁模型正確、互不干擾
- ~~交棒後另開 claude.ai 新對話，帳號預設模型不變~~ **驗收目標作廢**：實測 claude.ai 以「上次使用的模型」為新對話預設，本項前提不成立（見 §2.1 驗收後修正）
- [ ] 未設定的動作（維持預設）→ 開的分頁網址不帶 `?model=`，行為與改動前完全相同（未測，使用者評估與本功能關聯小；程式路徑為空字串直接回原網址，風險低）
- [ ] 備援模擬（暫時改壞輸入框選擇器）→ `?q=` 備援網址同時帶 `model` 參數，模型與預填同時生效（未測，同上）
- [ ] 既有功能迴歸：模板自訂、字數上限、中文確認、網頁翻譯、YouTube 字幕行為不變（未測，同上）
