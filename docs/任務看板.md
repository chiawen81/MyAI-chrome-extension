...
<details>
<summary>#23 [Bug] Claude 助手：Chat／Cowork 改版後文字可填入但無法自動送出</summary>
...
- 已確認根因範圍：Claude 已將首頁改為 Chat／Cowork 共用，截圖實際落在 `/cowork/...`；現有實作只依 2026-07-11 的 Chat 舊版 DOM 驗證結果，以全頁 `button[aria-label]` 加 `/send/i` 尋找送出鈕，未處理模式差異。尚待 spike 判定是送出鈕 selector 改版、`execCommand('insertText')` 未同步新版前端狀態，或兩者同時發生。
- 預定修復方向：①交棒時確保進入一般 Chat（避免沿用 Cowork 模式）；②輸入與送出偵測限制在 composer 範圍，依目前 DOM 補強穩定 selector／事件流程；③點擊後驗證確實送出；④「已填入但未送出」改為不會出現在 Chrome 錯誤頁的資訊級紀錄；⑤驗收 Chat、Cowork 起始狀態、只填不送、自動送出、指定模型與 `?q=` 備援。

### 前置 spike（需要使用者在已登入的 claude.ai 執行）

請先開一個**空白 Chat**，按 `F12` → Console；若 Chrome 阻擋貼上，依 Console 提示手動輸入 `allow pasting`。貼上以下腳本（腳本只填測試文字，不會點擊送出）：
...
(() => {
  ...
  const scan = (stage = 'scan') => {
    ...
    const buttons = [...document.querySelectorAll('button')]
      ...
      .filter((button) => button.nearEditor || button.type === 'submit' || /send|送出|傳送/i.test(`${button.ariaLabel} ${button.title} ${button.text}`));
  ...
  scan('initial');
  console.log('下一步：執行 btClaudeSpike.fill()；一秒後在輸入框末尾人工鍵入 x，再執行 btClaudeSpike.scan("after manual x")');
})();
```
...
2. 在輸入框文字末尾**人工鍵入一個 `x`**，不要送出。
3. Console 輸入 `btClaudeSpike.scan('after manual x')`。
4. 複製兩次表格輸出或截圖，並清掉測試文字。
5. 切換成一個**空白 Cowork**，重新貼上腳本並重做步驟 1–4。

回報時請附 Chat 與 Cowork 各自的 URL、`1s after execCommand`、`after manual x` 三項輸出；若程式填入後沒有送出鈕、人工鍵入 `x` 後才出現，便可確認是編輯器狀態同步問題。
