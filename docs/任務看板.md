...
<details>
<summary>#19 [Bug] YouTube 面板：換片後不會收合，字幕軌下拉變成空清單</summary>

- 回報（2026-07-13，#18 驗收時發現）：影片 A 開著「譯」面板 → 站內切到影片 B，面板留在畫面上且下拉選單沒有任何字幕軌選項；預期換片時收合面板、或自動更新為新影片的字幕軌。
- 根因已定位（`src/content/youtube/index.ts`）：換片時 `teardownSession()` 先把 `session` 設為 null 才呼叫 `updatePanelForNewVideo()` → 面板開著時觸發的 `populateTracks()` 因 `session` 為 null 提前 return，清單清空後不再回填；新 session 由其後的 `setupForVideo()` 建立，但沒有再觸發面板刷新。
- 修法方向：把「重置面板＋回填清單」移到 `setupForVideo()` 之後執行（離開 watch 頁時則直接收合面板）。單檔、但改變對外行為 → 走場景四輕量 Fix 計畫。
- 屬既有 bug（#18 的改動未觸及 index.ts），非 #18 迴歸。

</details>

<details>
<summary>#20 [優化] YouTube 字幕：首批翻譯完成前等待過久（實測 15～58 秒才見譯文）</summary>

- 回報（2026-07-13，#18 驗收時回饋）：開啟雙語字幕後長時間只顯示原文，容易誤以為翻譯失敗；三部影片首批（40 句）分別約 58／15 秒才出現譯文。
- 現況：批次預設 40 句**循序**送翻，首批完成前字幕層只有原文；首批延遲＝單次 LLM 翻完 40 句的時間，變異大（受供應商／模型／句長影響）。
- 方向（可組合，效益由大到小）：
  1. **批次並行化**：批次 context 用的是前一批的「原文」而非譯文（`translateAllCues`），批次間並無資料依賴，可比照功能一「小組並行、回來即渲染」（受並發數限制）——總時長從「各批相加」變「最慢一批」。
  2. **首批縮小**：首批 8～12 句、後續維持設定值（使用者提議 40→20 的強化版），首句譯文可在數秒內出現。
  3. **逐批寫入快取**：現在整部翻完才寫快取，中途換片／關閉全部作廢重來（FEATURES 已知限制），改逐批累積寫入。
  4. 從目前播放位置優先翻譯（實作較複雜，先評估 1＋2 效果再說）。
...
- 編號為全域流水號，跨欄位不重複、刪除不回收；**下一個編號：#21**（新增項目後記得遞增此數字）。
