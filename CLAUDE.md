...
需求規格：REQUIREMENTS.md（v0.1 原始規格，多數條目已實作完成、細節以 docs/FEATURES.md 與 docs/ARCHITECTURE.md 為準；0-A 合規約束已摘入下方「關鍵規則」，要查原始功能設想／明確捨棄範圍／MVP 驗收條件時才讀取）
...
- 改動跨執行環境通訊、訊息型別、或 background／content script 注入邏輯前，
  務必先讀 docs/ARCHITECTURE.md 第 1 節「跨環境須知」——記錄多個已修過的坑
...
- docs/BOARD.md — 任務看板（發想／待辦／進行中／已完成）。**讀取時機**：使用者提到新想法／bug，或要盤點任務狀態時
- docs/ARCHITECTURE.md — 執行環境職責、message passing 一覽、chrome.storage 資料結構、Provider 抽象層與擴充點、目錄結構。**讀取時機**：改動跨執行環境通訊、新增 message／storage key／Provider，或碰 background／content script 邏輯前（第 1 節跨環境須知另有「關鍵規則」明文提醒，見上）；純改單一模組內部邏輯（如樣式調整、單一函式）通常不需要
- docs/DEVELOPMENT.md — 命名規則、環境需求與指令、新增 AI 專家模板／譯文樣式／Provider／Claude 助手動作的具體步驟、計畫與 spike 流程。**讀取時機**：做上述任一種擴充，或要查命名規則／環境指令時
- docs/FEATURES.md — 驗收條件對照＋各功能實際行為與已知限制。**讀取時機**：要回答「這功能現在支援什麼／有什麼限制」，或確認驗收狀態時
- docs/TESTING.md — vitest 單元測試規劃（scanner、快取 hash、批次對齊 fallback）＋手動驗收清單。**讀取時機**：要寫測試或執行手動驗收時
- docs/CHANGELOG.md — 變更紀錄。**讀取時機**：要回顧近期改了什麼、或確認某功能何時／為何加入時（近期異動也可用 `git log` 查，通常更快）
